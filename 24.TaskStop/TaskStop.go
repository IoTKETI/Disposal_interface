...
func Response(parameters *Parameters.Parameter) {
	...
	tempB := bodyRS{parameters.MicroserviceID(),parameters.ChangedOutputParameter()}
	b, _ := json.Marshal(tempB)
	j := regexp.MustCompile(`(\s*?{\s*?|\s*?,\s*?)(['"])?([a-zA-Z0-9]+)(['"])?:`).ReplaceAll(b, []byte("$1$3="))
...
func RSparsing(data []byte,parameters *Parameters.Parameter)  {
	...
	s = re.ReplaceAll(btemp,[]byte("$1\"$3\""))
	ab = regexp.MustCompile(`(\s*?{\s*?|\s*?,\s*?)(['"])?([a-zA-Z0-9]+)(['"])?:`).ReplaceAll(s,[]byte("$1\"$3\":"))
	ab = regexp.MustCompile(`(\s*?:\s*?|\s*?}\s*?)(['"])?([a-zA-Z0-9]+)(['"])?}`).ReplaceAll(ab,[]byte("$1\"$3\"}"))
	//fmt.Println(string(ab))
	...
	parameters.SetMicroserviceID(barray.Mi)
	parameters.SetChangedOutputParameter(barray.Cop)
	fmt.Printf("\tParsing Completed: 'mi'=> %d, 'cop'=> %v \r\n",parameters.MicroserviceID(),parameters.ChangedOutputParameter())
